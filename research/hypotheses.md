...
- **H2 — Multi-method isolation:** two distinct methodology plugins can consume the same
  ...
  boundary.
- **H3 — Governed requirement feedback:** accepted findings from either evaluated methodology
  can justify governed Security Requirements while preserving a navigable provenance chain to
  the protected Functional Requirement, Base Analysis subjects, Analysis Record and
  method-specific evidence.
- **H4 — Change awareness:** baseline-aware provenance can identify dependent analytical
  artifacts that require re-evaluation after a relevant source or methodology change without
  invalidating unrelated artifacts.
