...
%<PAGE-HASH-DEFS>
\expandafter\def\csname PageMDfive1\endcsname{b28c70b50d3c12b8288d20b014599a0f}
...
\expandafter\def\csname PageMDfive39\endcsname{d5d7d0db53aaf3407a0115a330c98c02}
\expandafter\def\csname PageMDfive40\endcsname{2948054419b2fda2e7070f5b20f9da9c}
\expandafter\def\csname PageMDfive41\endcsname{e2240ebc51d9904f3871fe84395cb8af}
\expandafter\def\csname PageMDfive42\endcsname{2e3701287182209225d02fbea0547c4c}
\expandafter\def\csname PageMDfive43\endcsname{b5edd7fc3610ec76ec4b2992c199d786}
\expandafter\def\csname PageMDfive44\endcsname{b4acc8dbd83e82b51a0c9ba5c1aba411}
\expandafter\def\csname PageMDfive45\endcsname{df70ba8f961369f44bdd2b8e064a53bd}
\expandafter\def\csname PageMDfive46\endcsname{c275bc9695c3bd0921b63c6a01ef59ea}
%</PAGE-HASH-DEFS>
...
{\scriptsize
\textbf{Attivi:} \nolinkurl{DDTA_DOCUMENTATION_AUTHORING_GUIDE_R7_REBUILD_R8.tex}; \nolinkurl{DDTA_BASE_ANALYSIS_GUIDE_REBUILD_R7.tex}; questo \nolinkurl{DDTA_DERMATRIAGE_PARALLEL_CASE_STUDY_R25_FR03_SPLIT_REVIEW_R1.tex}. Working plan: \nolinkurl{DDTA_R25_BASE_ANALYSIS_GUIDE_REBUILD_WORK_PLAN_R6.md}.\par
\textbf{Riferimenti BA:} \nolinkurl{DDTA_BASE_ANALYSIS_OPERATIONAL_GUIDE_R3.tex} + BA0--BA5 e \nolinkurl{DDTA_BASE_ANALYSIS_GUIDE_REBUILD_R7.tex}. \textbf{Fase corrente:} R6 avvia il source-preservation audit completo; le correzioni documentali precedono la riesecuzione della BA e il successivo audit del grafo. Lo split \texttt{FR-MR01-03-03A/B} resta provvisorio e non canonico.\par
...
%<PAGE-DESC:39>MR-04 > DEC-10 - Qualificazione del disaccordo clinico come evidence per l'adattamento classificatorio
%<PAGE-CONTENT:39>
\PageHashFooter{\PageMDfive{39}}
\subsection{DEC-10 - Qualificazione del disaccordo clinico come evidence per l'adattamento classificatorio}
{\sffamily\scriptsize\color{DDTAGray}Gerarchia: \texttt{MR-04} $\rightarrow$ \texttt{DEC-10}}\par
...
\textbf{Context}\par
Non ogni revisione clinica costituisce evidence equivalente per l'adattamento della classificazione di urgenza. Le revisioni che esprimono disaccordo rispetto all'esito AI pertinente devono restare distinguibili dalle altre revisioni cliniche.

\textbf{Decision}\par
Per il percorso di adattamento della classificazione di urgenza, DermaTriage tratta come evidence qualificante sulla dimensione di disaccordo soltanto le revisioni cliniche che esprimono \texttt{ClinicianDisagreement} rispetto all'esito AI pertinente.

\textbf{Consequences}\par
\texttt{FR-07} governa la qualificazione operativa. \texttt{ClinicianDisagreement} e' il concetto governato; il data-state corrente \texttt{agrees == False} e' un binding della realizzazione B4 e non sostituisce l'identita' normativa del concetto.
\end{ddtacore}

\begin{ddtaquestions}[top=3pt,bottom=3pt,left=5pt,right=5pt]
\fontsize{4.05}{4.45}\selectfont
\begin{itemize}
  \item La relazione completa fra validation, correction e \texttt{agrees} resta \texttt{NOT SPECIFIED}; non viene ricostruito uno state model piu' forte di quanto documentato.
  \item La Decision stabilisce quale evidence e' qualificante; endpoint, retrieval e binding tecnici sono documentati nel FunctionalRequirement che realizza la scelta.
  \item Il consumo downstream dell'evidence da parte di \texttt{FR-05} non implica automaticamente deployment, retraining o trasferimenti ulteriori.
\end{itemize}
\end{ddtaquestions}
...
\begin{ddtareferentidentified}[top=3pt,bottom=3pt,left=5pt,right=5pt]
\fontsize{4.3}{4.7}\selectfont
\BACandidate{ClinicianDisagreement} \BANew{}; \BACandidate{ClassifierAdaptationEvidence} \BANew{}.
\end{ddtareferentidentified}

\begin{ddtareferentaccepted}[top=3pt,bottom=3pt,left=5pt,right=5pt]
\fontsize{4.25}{4.65}\selectfont
\BAAccepted{DermaTriage}; \BAAccepted{ClassifierAdaptationPath}; \BAAccepted{ClinicalReviewResult}; \BAAccepted{OriginalDermaTriageOutcome}.
\end{ddtareferentaccepted}

\begin{ddtapropositionidentified}[top=3pt,bottom=3pt,left=5pt,right=5pt]
\fontsize{3.95}{4.32}\selectfont
\textbf{Qualification / selection pressure.}\par
La Decision governa una distinzione di membership: soltanto le review che esprimono \texttt{ClinicianDisagreement} rispetto all'esito AI pertinente possono costituire evidence classificatoria. Non viene creata una proposition top-level \texttt{classify}, \texttt{constrain}, \texttt{condition} o \texttt{decisionRule}; \texttt{FR-07} possiede il comportamento operativo che rende disponibile l'evidence qualificata.
\end{ddtapropositionidentified}
...
\begin{ddtaopenrelations}[top=3pt,bottom=3pt,left=5pt,right=5pt]
\fontsize{3.8}{4.16}\selectfont
La semantica di qualification resta candidata a una struttura locale di \texttt{selection}: population = \texttt{ClinicalReviewResult}; criterion = \texttt{ClinicianDisagreement} rispetto al pertinente \texttt{OriginalDermaTriageOutcome}; selected result = \texttt{ClassifierAdaptationEvidence}. La struttura non e' ancora vocabulary BA ammesso. Il trasferimento B4 $\rightarrow$ DermaTriage del \texttt{ValidatedOutcome} e' un fatto separato gia' governato e verra' riusato in \texttt{FR-07}, senza duplicare la proposition esistente.
\end{ddtaopenrelations}
...
%<PAGE-DESC:40>MR-04 > DEC-10 > FR-07 - Qualificazione del disaccordo clinico come evidence per l'adattamento classificatorio
%<PAGE-CONTENT:40>
\PageHashFooter{\PageMDfive{40}}
\subsubsection{FR-07 - Qualificazione del disaccordo clinico come evidence per l'adattamento classificatorio}
{\sffamily\scriptsize\color{DDTAGray}Gerarchia: \texttt{MR-04} $\rightarrow$ \texttt{DEC-10} $\rightarrow$ \texttt{FR-07}}\par
...
\textbf{Functional clauses / inherited \texttt{normativeClause [1..*]}}\par
Nel determinare l'evidence clinica qualificante per il percorso di adattamento della classificazione di urgenza, DermaTriage MUST considerare qualificanti soltanto le revisioni cliniche che esprimono \texttt{ClinicianDisagreement} rispetto all'esito AI pertinente. Nella realizzazione B4 corrente tale stato e' rappresentato da \texttt{agrees == False}.

\textbf{Current realization / implementation bindings}\par
\begin{tabularx}{\linewidth}{L{55mm}Y}
\toprule
\textbf{Binding} & \textbf{Ruolo documentato} \\
\midrule
\texttt{GET /admin/retraining/fetch} & operazione DermaTriage protetta che recupera validated outcomes da B4 \\
\texttt{GET /api/v1/triage/list} & popolazione B4 dei consulti/casi usata dal lifecycle classificatorio \\
\texttt{GET /api/v1/chatbot/consulto/\{id\}/validated-outcome} & retrieval dell'outcome validato del consulto \\
\texttt{X-API-Key / bearer JWT} & binding di autenticazione gia' governati rispettivamente da \texttt{FR-24} e \texttt{FR-25} \\
\bottomrule
\end{tabularx}

\textbf{SPO references (L2 support)}\par
\texttt{ClinicalReview -- expresses --> ClinicianDisagreement}\par
\texttt{ClinicianDisagreement -- qualifies as --> ClassifierAdaptationEvidence}\par
\texttt{B4 -- provides --> ValidatedOutcome}\par
...
\begin{ddtaquestions}[top=3pt,bottom=3pt,left=5pt,right=5pt]
\fontsize{3.68}{4.03}\selectfont
\begin{itemize}
  \item Il binding semantico completo fra \texttt{ValidatedOutcome} e \texttt{ClinicalReviewResult} resta \texttt{NOT SPECIFIED}; l'esistenza del transfer B4 non autorizza a identificarli.
  \item La relazione completa fra validation, correction e \texttt{agrees} resta \texttt{NOT SPECIFIED}. \texttt{agrees == False} e' current representation di \texttt{ClinicianDisagreement}, non uno state model completo della review.
  \item Gli endpoint documentano acquisizione e retrieval; non vengono inferiti storage locale, retry, ordering, deduplication o ulteriori conveyance.
\end{itemize}
\end{ddtaquestions}
...
\begin{ddtareferentidentified}[top=3pt,bottom=3pt,left=5pt,right=5pt]
\fontsize{4.0}{4.38}\selectfont
\BACandidate{ClinicianDisagreement}; \BACandidate{ClassifierAdaptationEvidence}.
\end{ddtareferentidentified}

\begin{ddtareferentaccepted}[top=3pt,bottom=3pt,left=5pt,right=5pt]
\fontsize{3.78}{4.13}\selectfont
\BAAccepted{DermaTriage}; \BAAccepted{B4}; \BAAccepted{ValidatedOutcome}; \BAAccepted{ClinicalReviewResult}; \BAAccepted{OriginalDermaTriageOutcome}; \BAAccepted{ClassifierAdaptationPath}; \BAAccepted{ClinicianDisagreement} \BAFirstAccepted{}; \BAAccepted{ClassifierAdaptationEvidence} \BAFirstAccepted{}.
\end{ddtareferentaccepted}

\begin{ddtapropositionidentified}[top=3pt,bottom=3pt,left=5pt,right=5pt]
\fontsize{3.42}{3.75}\selectfont
\textbf{Selection semantics - identified, not consolidated.}\par
\texttt{candidatePopulation -> ClinicalReviewResult}\par
\texttt{membershipCriterion -> ClinicianDisagreement relative to OriginalDermaTriageOutcome}\par
\texttt{selectedResult -> ClassifierAdaptationEvidence}\par
\medskip
La membership qualification e' materialmente necessaria, ma \texttt{selection} resta una struttura locale / second-level non ancora ammessa nel vocabulary BA corrente; non viene quindi accettata come proposition autonoma.
\end{ddtapropositionidentified}

\begin{ddtapropositionaccepted}[top=3pt,bottom=3pt,left=5pt,right=5pt]
\fontsize{3.32}{3.64}\selectfont
\texttt{BAP-FR23-03} \emph{[riuso]}\par
\texttt{semanticOperatorKey = transfer}\par
\hspace*{1em}\texttt{source -> B4}\par
\hspace*{1em}\texttt{destination -> DermaTriage}\par
\hspace*{1em}\texttt{content -> ValidatedOutcome}\par
\texttt{polarity = affirmative}\par
\medskip
\texttt{BAP-FR07-01}\par
\texttt{semanticOperatorKey = produce}\par
\hspace*{1em}\texttt{actor -> DermaTriage}\par
\hspace*{1em}\texttt{input -> ClinicalReviewResult [0..*]}\par
\hspace*{1em}\texttt{result -> ClassifierAdaptationEvidence}\par
\texttt{polarity = affirmative}\par
\medskip
\emph{Acceptance boundary: il transfer non viene duplicato, ma riusa la proposition gia' accettata in \texttt{FR-23}. Il \texttt{produce} governa la disponibilita' dell'evidence qualificata; la membership rule resta finding locale non consolidato.}
\end{ddtapropositionaccepted}

\begin{ddtaopenrelations}[top=3pt,bottom=3pt,left=5pt,right=5pt]
\fontsize{3.38}{3.71}\selectfont
\texttt{ValidatedOutcome <semantic binding NOT SPECIFIED> ClinicalReviewResult}. Il transfer risponde alla provenienza del dato; il \texttt{produce} risponde alla costituzione dell'evidence qualificata dalla popolazione di review. \texttt{agrees == False} resta implementation binding di \texttt{ClinicianDisagreement} e non diventa BAReferent. Nessuna \texttt{observe/read}, \texttt{classify}, \texttt{constrain}, \texttt{condition} o \texttt{decisionRule} aggiuntiva viene introdotta.
\end{ddtaopenrelations}
...
%<PAGE-DESC:41>MR-04 > DEC-11 - Uso della priorita clinicamente corretta come sorgente del target di supervisione classificatoria
%<PAGE-CONTENT:41>
\PageHashFooter{\PageMDfive{41}}
\subsection{DEC-11 - Uso della priorita clinicamente corretta come sorgente del target di supervisione classificatoria}
{\sffamily\scriptsize\color{DDTAGray}Gerarchia: \texttt{MR-04} $\rightarrow$ \texttt{DEC-11}}\par
...
\textbf{Authority} & \texttt{CURRENT\_GOVERNED} \\
\textbf{Type} & \texttt{Decision} \\
\textbf{Title} & Uso della priorita clinicamente corretta come sorgente del target di supervisione classificatoria \\
\textbf{Parent MacroRequirement} & \texttt{MR-04} \\
\textbf{Decision kind} & \texttt{SELECTABLE} \\
\bottomrule
\end{tabularx}
...
\textbf{Context}\par
L'adattamento della classificazione di urgenza richiede un target di supervisione coerente con l'esito clinicamente corretto del caso. La priorita operativa P-scale corretta dal professionista sanitario costituisce il riferimento clinico dal quale derivare il target utilizzato dal percorso di adattamento classificatorio.

\textbf{Decision}\par
Per l'adattamento della classificazione di urgenza, DermaTriage usa la priorita operativa P-scale clinicamente corretta come sorgente semantica del target di supervisione del classificatore.

\textbf{Consequences}\par
La priorita operativa P-scale clinicamente corretta e il target di supervisione classificatoria restano significati distinti. La derivazione del target di supervisione costituisce un comportamento funzionale distinto dal successivo retraining del classificatore. Il target derivato mantiene la pertinenza al caso clinico cui appartiene la priorita corretta.
\end{ddtacore}

\begin{ddtaquestions}[top=3pt,bottom=3pt,left=5pt,right=5pt]
\fontsize{4.0}{4.4}\selectfont
\begin{itemize}
  \item La Decision governa la sorgente semantica del target; il mapping concreto P-scale $\rightarrow$ target resta comportamento funzionale del requisito sottostante.
  \item La priorita clinicamente corretta e il target derivato non vengono identificati come lo stesso significato, anche quando restano associati allo stesso caso.
  \item Il successivo consumo del target nel retraining non prova da solo una conveyance interna con source e destination distinti.
\end{itemize}
\end{ddtaquestions}
...
\begin{ddtareferentidentified}[top=3pt,bottom=3pt,left=5pt,right=5pt]
\fontsize{4.25}{4.65}\selectfont
\BACandidate{ClinicallyCorrectedOperationalTriagePriority} \BANew{}; \BACandidate{ClassifierUrgencyTarget} \BANew{}.
\end{ddtareferentidentified}

\begin{ddtareferentaccepted}[top=3pt,bottom=3pt,left=5pt,right=5pt]
\fontsize{4.35}{4.75}\selectfont
\BAAccepted{DermaTriage}; \BAAccepted{ClassifierAdaptationPath}.
\end{ddtareferentaccepted}

\begin{ddtapropositionidentified}[top=3pt,bottom=3pt,left=5pt,right=5pt]
\fontsize{3.92}{4.3}\selectfont
\textbf{Derivation pressure.}\par
La Decision governa quale significato clinico alimenta il target di supervisione e mantiene distinti source semantic e result semantic. Non viene accettata una BAProposition autonoma a livello Decision: il comportamento produttivo concreto e il mapping sono posseduti dal FunctionalRequirement sottostante.
\end{ddtapropositionidentified}
...
\begin{ddtaopenrelations}[top=3pt,bottom=3pt,left=5pt,right=5pt]
\fontsize{3.78}{4.14}\selectfont
La stessa \texttt{ClinicallyCorrectedOperationalTriagePriority} resta la sorgente semantica del \texttt{ClassifierUrgencyTarget} che verra' consolidato nel FunctionalRequirement. La Decision non introduce un \texttt{transfer}, un \texttt{realize} o una \texttt{transition}: provenienza B4, derivazione operativa e consumo downstream restano fatti distinti da preservare ai rispettivi boundary.
\end{ddtaopenrelations}
...
%<PAGE-DESC:42>MR-04 > DEC-11 > FR-08 - Derivazione del target di supervisione dalla priorita P-scale corretta
%<PAGE-CONTENT:42>
...
\textbf{Authority} & \texttt{CURRENT\_GOVERNED} \\
\textbf{Type} & \texttt{FunctionalRequirement} \\
\textbf{Title} & Derivazione del target di supervisione dalla priorita P-scale corretta \\
...
\textbf{Functional obligation}\par
Per i casi destinati all'adattamento classificatorio, DermaTriage deriva il target di supervisione del classificatore dalla priorita operativa P-scale clinicamente corretta associata al caso. Nel workflow integrato, il \texttt{ValidatedOutcome} necessario al percorso di adattamento viene trasferito da B4 a DermaTriage; la relazione strutturale completa fra \texttt{ValidatedOutcome} e priorita P-scale clinicamente corretta non e' ulteriormente specificata e i due significati non vengono trattati come equivalenti. Nella realizzazione corrente, il target derivato e' utilizzato dal Continuous Retraining Protocol di EfficientNet-B4; la pipeline di auto-retraining e' implementata in \texttt{image\_classifier/retrainer.py}. I target \texttt{HIGH}, \texttt{MEDIUM}, \texttt{LOW} sono codificati correntemente con gli indici \texttt{0}, \texttt{1}, \texttt{2}; tale codifica e' un binding della realizzazione e non modifica l'identita funzionale del target. Non e' definita un'identita separata di funzione o metodo Python dedicato esclusivamente alla derivazione P-scale $\rightarrow$ target di supervisione.
...
\textbf{SPO references (L2 support)}\par
\texttt{B4 -- transfers --> ValidatedOutcome}\par
\texttt{ValidatedOutcome -- to --> DermaTriage}\par
\texttt{ClinicallyCorrectedOperationalTriagePriority -- derives --> ClassifierUrgencyTarget}\par
\texttt{DermaTriage -- derive --> ClassifierUrgencyTarget}\par
\texttt{ClassifierUrgencyTarget -- consumed by --> EfficientNet-B4 retraining}
\end{ddtacore}

\begin{ddtaquestions}[top=3pt,bottom=3pt,left=5pt,right=5pt]
\fontsize{3.58}{3.93}\selectfont
\begin{itemize}
  \item Il binding semantico completo fra \texttt{ValidatedOutcome} e \texttt{ClinicallyCorrectedOperationalTriagePriority} resta \texttt{NOT SPECIFIED}; l'esistenza del transfer B4 non autorizza a identificarli.
  \item Il comportamento per priorita corretta assente, invalida, fuori dominio o conflittuale non e' specificato.
  \item Non e' definito un handoff interno con source/destination sufficientemente distinti per promuovere il consumo di \texttt{ClassifierUrgencyTarget} da parte del retraining a un ulteriore transfer.
\end{itemize}
\end{ddtaquestions}
...
\begin{ddtareferentidentified}[top=3pt,bottom=3pt,left=5pt,right=5pt]
\fontsize{4.0}{4.38}\selectfont
\BACandidate{ClinicallyCorrectedOperationalTriagePriority}; \BACandidate{ClassifierUrgencyTarget}.
\end{ddtareferentidentified}

\begin{ddtareferentaccepted}[top=3pt,bottom=3pt,left=5pt,right=5pt]
\fontsize{3.72}{4.08}\selectfont
\BAAccepted{DermaTriage}; \BAAccepted{ClassifierAdaptationPath}; \BAAccepted{B4}; \BAAccepted{ValidatedOutcome}; \BAAccepted{EfficientNet-B4}; \BAAccepted{ClinicallyCorrectedOperationalTriagePriority} \BAFirstAccepted{}; \BAAccepted{ClassifierUrgencyTarget} \BAFirstAccepted{}.
\end{ddtareferentaccepted}

\begin{ddtapropositionidentified}[top=3pt,bottom=3pt,left=5pt,right=5pt]
\fontsize{3.38}{3.72}\selectfont
\textbf{Mapping structure / transfer boundary.}\par
Il mapping P-scale $\rightarrow$ target e' una struttura locale della \texttt{produce} accettata sotto; la candidate \texttt{operatorStructure.decisionRule} non e' una proposition autonoma. Il transfer B4 $\rightarrow$ DermaTriage del \texttt{ValidatedOutcome} riusa \texttt{BAP-FR23-03}. Il consumo del \texttt{ClassifierUrgencyTarget} nel retraining non viene trasformato in un secondo transfer: source e destination interne non sono sufficientemente governate.
\end{ddtapropositionidentified}

\begin{ddtapropositionaccepted}[top=3pt,bottom=3pt,left=5pt,right=5pt]
\fontsize{3.08}{3.39}\selectfont
\texttt{BAP-FR23-03} \emph{[riuso]}\par
\texttt{semanticOperatorKey = transfer}\par
\hspace*{1em}\texttt{source -> B4}\par
\hspace*{1em}\texttt{destination -> DermaTriage}\par
...
\hspace*{1em}\texttt{operatorStructure [CANDIDATE R7 / NOT ADMITTED]}\par
\hspace*{2em}\texttt{decisionRule}\par
\hspace*{3em}\texttt{P1 -> HIGH; P2 -> HIGH; P3 -> MEDIUM; P4 -> LOW}\par
\texttt{polarity = affirmative}\par
\emph{Acceptance boundary: la \texttt{produce} e il transfer upstream gia' accettato sono preservati come fatti distinti. La struttura di mapping resta locale/candidate e non introduce un operatore top-level aggiuntivo.}
\end{ddtapropositionaccepted}

\begin{ddtaopenrelations}[top=3pt,bottom=3pt,left=5pt,right=5pt]
\fontsize{3.28}{3.6}\selectfont
\texttt{ValidatedOutcome <semantic binding NOT SPECIFIED> ClinicallyCorrectedOperationalTriagePriority}. Gli indici \texttt{0/1/2} e \texttt{image\_classifier/retrainer.py} sono binding della realizzazione corrente e non diventano automaticamente BAReferent ne' una proposition \texttt{realize} della derivazione. \texttt{ClassifierUrgencyTarget -> retraining} resta consumo downstream documentato, non una conveyance BA autonoma.
\end{ddtaopenrelations}

\end{paracol}
%</PAGE-CONTENT:42>
\clearpage
%<PAGE-DESC:43>Registro Base Analysis - BAReferent e BAProposition
%<PAGE-CONTENT:43>
...
\midrule
DermaTriage & \texttt{MR-01}; \textbf{\texttt{DEC-MR01-01}}; \texttt{FR-MR01-01-01}; \texttt{DEC-MR01-03}; \texttt{FR-MR01-03-01}; \texttt{FR-MR01-03-02}; \texttt{FR-MR01-03-04}; \texttt{DEC-MR01-04}; \texttt{FR-MR01-04-01}; \texttt{MR-02}; \texttt{MR-03}; \texttt{DEC-03}; \texttt{FR-03}; \texttt{FR-12}; \texttt{DEC-15}; \texttt{FR-23}; \texttt{DEC-16}; \texttt{FR-24}; \texttt{FR-25}; \texttt{MR-04}; \texttt{DEC-04}; \texttt{FR-04}; \texttt{FR-05}; \texttt{DEC-05}; \texttt{FR-09}; \texttt{DEC-09}; \texttt{FR-06}; \texttt{DEC-10}; \texttt{FR-07}; \texttt{DEC-11}; \texttt{FR-08} \\
...
EfficientNet-B4 & \textbf{\texttt{FR-MR01-03-01}}; \texttt{MR-04}; \texttt{FR-05}; \texttt{DEC-06}; \texttt{DEC-08}; \texttt{FR-10}; \texttt{FR-08} \\
...
ClassifierAdaptationPath & \textbf{\texttt{MR-04}}; \texttt{DEC-04}; \texttt{FR-05}; \texttt{DEC-05}; \texttt{FR-13}; \texttt{FR-14}; \texttt{FR-15}; \texttt{DEC-10}; \texttt{FR-07}; \texttt{DEC-11}; \texttt{FR-08} \\
...
ClinicallyCorrectedOperationalTriagePriority & \texttt{DEC-11}; \textbf{\texttt{FR-08}} \\
ClassifierUrgencyTarget & \texttt{DEC-11}; \textbf{\texttt{FR-08}} \\
SpecialistDestinationIndication & \textbf{\texttt{MR-02}} \\
ClinicalReviewResult & \textbf{\texttt{MR-03}}; \texttt{DEC-03}; \texttt{FR-03}; \texttt{FR-12}; \texttt{MR-04}; \texttt{DEC-09}; \texttt{FR-06}; \texttt{DEC-10}; \texttt{FR-07} \\
OriginalDermaTriageOutcome & \textbf{\texttt{MR-03}}; \texttt{DEC-03}; \texttt{FR-03}; \texttt{FR-12}; \texttt{FR-23}; \texttt{DEC-10}; \texttt{FR-07} \\
HealthcareProfessional & \textbf{\texttt{MR-03}} \\
ClinicalReviewDisposition & \textbf{\texttt{FR-12}} \\
ClinicianDisagreement & \texttt{DEC-10}; \textbf{\texttt{FR-07}} \\
ClassifierAdaptationEvidence & \texttt{DEC-10}; \textbf{\texttt{FR-07}} \\
B4 & \textbf{\texttt{DEC-15}}; \texttt{FR-23}; \texttt{DEC-16}; \texttt{FR-25}; \texttt{FR-07}; \texttt{FR-08} \\
B4TriageOutcome & \texttt{DEC-15}; \textbf{\texttt{FR-23}} \\
ClinicianValidation & \texttt{DEC-15}; \textbf{\texttt{FR-23}} \\
ValidatedOutcome & \texttt{DEC-15}; \textbf{\texttt{FR-23}}; \texttt{FR-07}; \texttt{FR-08} \\
...
%</PAGE-CONTENT:43>
\clearpage
%<PAGE-DESC:44>Registro Base Analysis - BAProposition
%<PAGE-CONTENT:44>
...
\textbf{\texttt{BAP-FR23-03}} & \texttt{FR-23}; \texttt{FR-07}; \texttt{FR-08} -- \texttt{transfer} (\texttt{B4 -> DermaTriage : ValidatedOutcome}); \texttt{FR-07} e \texttt{FR-08} riusano la conveyance upstream gia\' accettata e non creano proposition duplicate \\
...
\textbf{\texttt{BAP-FR07-01}} & \texttt{FR-07} -- accepted \texttt{produce} (\texttt{DermaTriage}: available \texttt{ClinicalReviewResult} population $\rightarrow$ \texttt{ClassifierAdaptationEvidence}); membership criterion = \texttt{ClinicianDisagreement} relative to the pertinent \texttt{OriginalDermaTriageOutcome}; selection semantics retained as non-consolidated local finding; \texttt{BAP-FR23-03} reused for upstream B4 conveyance; \texttt{ValidatedOutcome <-> ClinicalReviewResult} binding remains \texttt{NOT SPECIFIED} \\
\textbf{\texttt{BAP-FR08-01-01}} & \texttt{FR-08} -- accepted \texttt{produce} (\texttt{DermaTriage}: corrected P-scale $\rightarrow$ classifier target); \texttt{BAP-FR23-03} riusata per la conveyance B4 upstream; candidate \texttt{operatorStructure.decisionRule} locale al mapping; nessun transfer interno target $\rightarrow$ retraining accettato \\
\bottomrule
\end{tabularx}
%</PAGE-CONTENT:44>
\clearpage
%<PAGE-DESC:45>MR-01 - Inventario temporaneo transfer, contract e decisioni BA [DA CANCELLARE]
%<PAGE-CONTENT:45>
...
\IntegrityRow{39}{MR-04 > DEC-10 - Qualificazione del disaccordo clinico come evidence per l'adattamento classificatorio}{\PageMDfive{39}}
\IntegrityRow{40}{MR-04 > DEC-10 > FR-07 - Qualificazione del disaccordo clinico come evidence per l'adattamento classificatorio}{\PageMDfive{40}}
\IntegrityRow{41}{MR-04 > DEC-11 - Uso della priorita clinicamente corretta come sorgente del target di supervisione classificatoria}{\PageMDfive{41}}
\IntegrityRow{42}{MR-04 > DEC-11 > FR-08 - Derivazione del target di supervisione dalla priorita P-scale corretta}{\PageMDfive{42}}
\IntegrityRow{43}{Registro Base Analysis - BAReferent e BAProposition}{\PageMDfive{43}}
\IntegrityRow{44}{Registro Base Analysis - BAProposition}{\PageMDfive{44}}
\IntegrityRow{45}{MR-01 - Inventario temporaneo transfer, contract e decisioni BA [DA CANCELLARE]}{\PageMDfive{45}}
