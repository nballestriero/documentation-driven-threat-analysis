...
\textbf{not canonical ThreatForge changes}. Earlier candidate
\texttt{Objective} sections are retained as study history; the current
MR working contract removes \texttt{Objective} and lets \texttt{Intent}
own macro purpose, value and desired outcome.
...
\item
  ...
  governare l\textquotesingle evoluzione del progetto.
\item
...
governati e di mantenere chiaro, durante l\textquotesingle evoluzione
del progetto, che cosa viene realizzato e con quali evidenze viene
verificato.
...
\item
  manutentori
\item
  utenti che usano ThreatForge per guidare l\textquotesingle authoring e
  l\textquotesingle implementazione
\end{itemize}
...
\item
  Offrire assistenza coerente all\textquotesingle authoring e
  all\textquotesingle esecuzione del lavoro senza spostare le regole di
  dominio dentro uno specifico strumento di sviluppo.
\end{itemize}
...
\item
  ...
  origin belongs to one of that document\textquotesingle s descendants.
\item
...
l\textquotesingle analisi}\label{proposed-mr-0003---modello-del-progetto-per-lanalisi}
...
\item
  Permettere analisi anticipate e ripetute durante
  l\textquotesingle evoluzione del progetto.
\item
...
\paragraph{Second reformulation (historical working candidate): MR-0003
- Governed representation of the
system}\label{second-reformulation-historical-working-candidate-mr-0003---governed-representation-of-the-system}
...
\item
  ...
  identificabile.
\item
  Mantenere separata l\textquotesingle evoluzione del progetto governato
  dall\textquotesingle evoluzione del motore ThreatForge.
\end{itemize}
...
\paragraph{Second reformulation (historical working candidate): MR-0005
- Governed analysis of the
system}\label{second-reformulation-historical-working-candidate-mr-0005---governed-analysis-of-the-system}
...
methods while preserving the method used, the analyst\textquotesingle s
interpretation, the evidence considered and the results produced.
...
\subsection{Intent versus Objective closure test across
ThreatForge}\label{intent-versus-objective-closure-test-across-threatforge}

The five original ThreatForge MRs and their candidate rewrites were used
as a second corpus after the four facial-access MRs. The result
reinforces removal of \texttt{Objective}.

\begin{itemize}
\tightlist
\item
  \textbf{MR-0001} - Original Intent already states the
  governed-documentation macro result; the long Macro obligation
  descends into registries, Markdown, taxonomies, Diataxis and
  projections. The candidate Objective restates reliable/traced
  documentation and starts adding design principles such as avoiding
  duplicated rules. \textbf{Result:} no independent Objective semantics.
\item
  \textbf{MR-0002} - Original Intent already states the
  Requirement-to-implementation/verification flow; the Macro obligation
  descends into preview, rollback, scaffolding, Git runner and editor
  adapters. The candidate Objective repeats traceability and adds
  tool-independence/design constraints. \textbf{Result:} keep the macro
  outcome in Intent and move lower details down.
\item
  \textbf{MR-0003} - Original Intent already states the
  documentation-derived methodology-neutral system representation;
  obligations specify identity, provenance, lifecycle and topology. The
  first Objective repeated analysis-readiness, while the second added
  the important fact/interpretation distinction. \textbf{Result:}
  preserve that distinction by strengthening Intent/Constraints, not by
  retaining Objective.
\item
  \textbf{MR-0004} - Original Intent already states governed Target
  Project lifecycle/isolation. The candidate Objective is almost a pure
  paraphrase of autonomy/isolation. \textbf{Result:} Objective is
  redundant.
\item
  \textbf{MR-0005} - Original Intent already states the common governed
  analysis model. The candidate Objective repeats multi-method analysis,
  traceability and deterministic repetition. \textbf{Result:} Objective
  is redundant/decompositional.
\end{itemize}

\subsubsection{Closed conceptual
decision}\label{closed-conceptual-decision}

\texttt{Objective} is removed from the MR conceptual model.
\texttt{Intent} is the concise owner of \textbf{macro purpose + value +
desired outcome}. If a separate Objective contains useful non-duplicated
material, that material is redistributed to Intent, Scope, Constraints,
Decisions or Requirements according to its semantics.

\subsubsection{Final candidate Intent set after Objective
removal}\label{final-candidate-intent-set-after-objective-removal}

\begin{itemize}
\tightlist
\item
  \textbf{MR-0001 - Documentazione di progetto governata e tracciabile}:
  Consentire a persone e strumenti di comprendere, mantenere e
  verificare le conoscenze che guidano un progetto attraverso
  documentazione versionata, coerente e tracciabile, mantenendo
  espliciti i collegamenti tra intenzioni, decisioni, requisiti ed
  evidenze.
\item
  \textbf{MR-0002 - Sviluppo tracciabile dai requisiti}: Consentire al
  lavoro di implementazione di partire da requisiti governati e di
  mantenere chiaro, durante l\textquotesingle evoluzione del progetto,
  che cosa viene realizzato, perche e con quali evidenze viene
  verificato.
\item
  \textbf{MR-0003 - Rappresentazione governata del sistema}: Rendere il
  sistema descritto dal progetto comprensibile attraverso conoscenza
  esplicita, tracciabile e mantenibile, distinguendo la conoscenza
  governata dalle integrazioni revisionate e dalle successive
  interpretazioni, affinche persone e attivita successive possano fare
  affidamento su una rappresentazione condivisa e giustificata del
  sistema.
\item
  \textbf{MR-0004 - Gestione isolata dei progetti governati}: Consentire
  a ThreatForge di creare, aprire e governare progetti distinti
  mantenendo separati i loro documenti, risultati e ciclo di vita dal
  motore ThreatForge e dagli altri progetti.
\item
  \textbf{MR-0005 - Analisi governata del sistema}: Consentire a metodi
  di analisi differenti di esaminare la conoscenza governata del sistema
  e produrre risultati revisionabili, tracciabili e ripetibili,
  preservando il metodo usato, l\textquotesingle interpretazione esperta
  e le evidenze senza modificare implicitamente la descrizione
  sottostante del sistema.
\end{itemize}

\subsection{Teachability-map observation from the final candidate
set}\label{teachability-map-observation-from-the-final-candidate-set}

The final candidate titles + Intent can be arranged into a didactic
project map that explains ThreatForge without exposing registries, BAE
records, plugins, DFD roles, editor adapters or other lower-level
mechanisms. The saved study map is in
\texttt{../04-teachability-maps/threatforge-macro-project-map.html}.

The current candidate MR corpus does not yet govern all map links as
explicit \texttt{dependsOn} relations. The study map therefore marks its
explanatory links as \textbf{non-canonical didactic interpretations} and
keeps traceability to the MR/ADR-family evidence. This is intentional:
the teachability test may reveal where an explicit relation would
improve the model, but the projection must not silently become topology
authority.
