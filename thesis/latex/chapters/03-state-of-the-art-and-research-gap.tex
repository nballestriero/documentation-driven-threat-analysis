...
Il confronto è costruito attorno alla domanda centrale della tesi: se un progetto possiede documentazione governata, è possibile derivarne una rappresentazione analizzabile e metodologicamente neutrale, permettere a metodi differenti di operare sulla stessa conoscenza di sistema e riportare i risultati accettati nella documentazione come requisiti di sicurezza tracciabili? La risposta non può essere ricavata da un singolo filone. Gli approcci requirements-first mostrano come partire da requisiti e scenari; gli approcci goal-oriented e problem-frame formalizzano intenzioni, contesto e assunzioni; il model-driven security sposta l'automazione su modelli espliciti; l'automated threat modelling applica cataloghi e regole dopo che un modello tipizzato è disponibile; l'automated requirements engineering affronta invece la trasformazione a monte da testo a artefatti candidati; la traceability literature studia come preservare relazioni e origine; gli studi più recenti sugli LLM mostrano infine che leggibilità, correttezza semantica, coverage, groundedness e riproducibilità sono proprietà distinte.
...
Il primo è una \textbf{frammentazione per boundary}. Le fonti coprono in modo convincente tratti specifici: scenario-to-threat, goal-to-anti-goal, model-to-threat, model-to-test, document-to-candidate-model, trace-link recovery o artifact-to-rerun. Nessuna fonte del corpus copre l'intera catena dalla documentazione governata fino al requisito di sicurezza con change impact end-to-end.
...
\subsection{G1 - Dalla documentazione a un modello analizzabile governato}

Gli approcci threat-oriented assumono generalmente un modello già pronto; gli approcci automated-RE possono generare artefatti da testo ma non stabiliscono una trasformazione completa, semanticamente affidabile, continuamente sincronizzata e methodology-neutral. Rimane quindi aperto il problema di costruire una rappresentazione canonica a partire dalla normale documentazione di progetto conservando, per ogni elemento derivato, fonte, versione, natura estratta o inferita e disposition di revisione.

In forma compatta:

\begin{center}
\textbf{raw/governed documentation $\not\equiv$ trusted neutral analysis model.}
\end{center}

La letteratura fornisce evidenza che la trasformazione sia possibile per artefatti specifici, non che essa sia già risolta per un core security-analyzable generale.
...
\textbf{governed documentation} $\rightarrow$ \textbf{reviewed methodology-neutral Base Analysis} $\rightarrow$ \textbf{isolated methodology overlay} $\rightarrow$ \textbf{method-specific Analysis Record} $\rightarrow$ \textbf{reviewed Common Finding} $\rightarrow$ \textbf{governed Security Requirement} $\rightarrow$ \textbf{change-aware re-analysis}
}}
\end{center}

Ogni tratto della catena è motivato da una debolezza osservata nelle fonti. La candidate layer è motivata dagli errori delle trasformazioni automatiche e LLM; la neutralità della Base Analysis è motivata dall'eterogeneità dei viewpoint; il plugin boundary è motivato dalla natura method-specific di cataloghi, failure mode e regole; il Common Finding evita di incorporare la tassonomia del metodo nel requisito; provenance e review derivano dai problemi di traceability e grounding; lo stale state deriva dalla necessità di mantenere valida l'analisi quando cambiano fonti e strumenti.

Questa formulazione evita due claim che la letteratura non permette ancora di sostenere. Primo, non si afferma che la documentazione governata sia sufficiente a derivare sempre una Base Analysis completa: questa è materia della RQ1. Secondo, non si afferma che un core neutrale supporti qualunque metodologia: la RQ2 verrà valutata attraverso STRIDE e STRIDE-AI e la generalizzabilità oltre questi due casi resterà un limite esplicito.
...
\item \textbf{Il gap è una composizione governata, non un nuovo metodo di threat identification.} Manca nel corpus un processo che colleghi documentazione governata, modello neutrale revisionato, più overlay metodologici, finding comuni, requisiti di sicurezza e re-analysis basata su provenance mantenendo separata la semantica dei diversi metodi.
\end{enumerate}
