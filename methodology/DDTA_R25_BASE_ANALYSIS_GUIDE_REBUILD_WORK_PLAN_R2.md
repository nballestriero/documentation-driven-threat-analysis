# DDTA R25 - Base Analysis Guide Rebuild and DermaTriage Parallel Application Work Plan R2
...
**New guide under construction:** `DDTA_BASE_ANALYSIS_GUIDE_REBUILD_R2`
...
The Rebuild R2 structural boundary and the temporary rebuild appendix are consolidated by this checkpoint. The next review unit is still **not an operator**.

Proceed with the first definitive-guide content in this order:

1. review and refine Section 1, `Che cos'e' la Base Analysis`;
2. introduce and review the first BA foundation, `governed semantic fact`;
3. continue with `BAReferent`;
4. continue with `BAProposition`;
5. apply each accepted foundation immediately to the already consolidated DermaTriage documentation before moving to the next one.

The Construct Description Contract remains in the temporary rebuild appendix and governs how later constructs are written. If application exposes an insufficiency in that contract, revise the contract explicitly and regression-check already reviewed constructs rather than changing the description format locally.
