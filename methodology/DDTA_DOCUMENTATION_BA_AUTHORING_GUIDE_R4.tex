...
\vspace{0.25cm}
{\LARGE Revision 4 - pre-holdout frozen documentation protocol\par}
\vspace{0.9cm}
{\large Metodo operativo per documentare il minimum sufficient governed meaning e consegnarlo alla Base Analysis\par}
\vfill
\begin{minipage}{0.88\textwidth}
\small
\textbf{Stato.} R25 DOCUMENTATION-METHOD FREEZE / PRE-HOLDOUT AUTHORING PROTOCOL.

\medskip
\textbf{Project authority.} \code{FACIAL-ACCESS-GOV-R2}.

\medskip
\textbf{Facial Access BA evidence.} \code{FACIAL-ACCESS-BA-R24-R1}: complete case BA / post-BA regression PASS; integrated BA6 acceptance OPEN.

\medskip
\textbf{Pinned source revision.} \code{8af2257a1df94fa5a83d4853ed0a1eb4d020c429}.

\medskip
Questa guida congela il protocollo documentale pre-holdout. Non sostituisce i documenti governati del progetto e non usa BA6 come risultato già chiuso. La Base Analysis resta una fase downstream separata.
\end{minipage}
\vfill
...
\section{R25 pre-holdout freeze}

Questa revisione R4 è il protocollo documentale congelato che verrà usato per il nuovo caso di validazione strutturalmente diverso.

\begin{closedbox}[Freeze rule]
Il nuovo holdout non è ancora selezionato né scritto. Di conseguenza, nessuna regola di questa guida è stata adattata al dominio futuro.

Dopo il freeze, il nuovo caso può:
\begin{itemize}
\item essere rappresentato correttamente dal metodo corrente;
\item esporre una insufficienza del contratto documentale;
\item esporre una distinzione non governata e quindi produrre \code{NOT SPECIFIED};
\item richiedere una riapertura solo attraverso un counterexample concreto.
\end{itemize}
\end{closedbox}

\subsection{Anti-contamination rule}

Durante l'authoring del holdout non si consulta BA2 per decidere quali significati di progetto introdurre.

Sono vietate motivazioni come:

\begin{lstlisting}
aggiungiamo un transfer per testare transfer
aggiungiamo uno state change per testare transition
creiamo una proprieta' per facilitare decisionRule
scegliamo una struttura perche' la BA la rappresenta bene
\end{lstlisting}

La documentazione deve essere guidata dal problema e dalle decisioni del progetto.

\subsection{Status delle regole}

La guida distingue:

\begin{lstlisting}
FROZEN PRE-HOLDOUT RULE
    regola usata per il nuovo caso

CASE EVIDENCE
    esempio Facial Access che illustra la regola

NOT SPECIFIED / OPEN
    significato non governato o domanda non ancora chiusa
\end{lstlisting}

Il freeze non implica completezza universale del metodo. Un nuovo counterexample puo' riaprire il minimo owning layer.
...
La BA applica poi il proprio criterio minimo e il contratto pre-holdout corrente (BA0--BA5 con BA2 R3). BA6 resta il successivo gate di validazione integrata.
...
La baseline governata dell'esempio è \code{FACIAL-ACCESS-GOV-R2}.

La Base Analysis \code{FACIAL-ACCESS-BA-R24-R1} è conservata come complete case BA con post-BA regression PASS. L'accettazione integrata BA6 resta \textbf{OPEN}.

Questa guida R4 è la \textbf{documentation-method baseline congelata pre-holdout}.

Il suo prossimo uso non è la scrittura della tesi né la selezione di una soluzione tecnica: è l'authoring del nuovo caso di validazione, ma soltanto dopo il completamento delle restanti fasi R25 pre-holdout.

Prima del nuovo caso devono ancora essere congelati:

\begin{itemize}
\item il contratto Base Analysis pre-holdout;
\item l'evidenza thesis-ready;
\item i pointer repository/tesi;
\item il checkpoint immutabile PRE-HOLDOUT.
\end{itemize}

ThreatForge resta differito alla fase post-stabilizzazione come reference implementation / experimental harness.

La valutazione STRIDE/STRIDE-AI resta successiva alla stabilizzazione metodologica e alla reimplementazione di ThreatForge.
\end{document}
