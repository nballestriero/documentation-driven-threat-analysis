...
\expandafter\def\csname PageMDfive83\endcsname{816d0bc660638027cd49ab41d5b87c2e}
\expandafter\def\csname PageMDfive84\endcsname{d8daf7031f5d8edb5455917e4fe6df90}
...
\begin{enumerate}
  ...
  \item usare \texttt{CDC-R1} come controllo di copertura e assumere le Sezioni 1--12 come base espositiva corrente della Rebuild R7;
  ...
  \item considerare \texttt{transfer}, \texttt{produce}, \texttt{realize} e \texttt{constrain} i primi quattro operatori completamente ricostruiti, \texttt{condition} il primo scoped modifier completamente ricostruito e \texttt{decisionRule} la prima struttura candidate;
  \item mantenere i discriminatori necessari fra applicability, result selection, allowed-domain constraint, dependency e identity pressure, preservando in \texttt{constrain} anche la distinzione fra constraint nominato/riusabile e restriction locale;
  \item per \texttt{decisionRule}, usare \texttt{eq}/\texttt{ne} come normalizzazione delle semantiche storiche e \texttt{gt} come sola estensione candidate attualmente giustificata; non completare per simmetria la famiglia dei comparator;
  \item preservare i branch residuali nella forma source-grounded e nel loro scope, senza convertirli in \texttt{ELSE}, negazioni o comparison complementari; missing/invalid/out-of-domain restano non specificati quando la documentazione non li governa;
  \item considerare superato il pressure test DermaTriage di \texttt{FR-MR01-04-01} per \texttt{gt} e residual preservation, senza trattare cio' come promotion della rilocazione strutturale;
  \item completare la regression sul caso Facial Access e il checkpoint esplicito prima di modificare l'authority top-level di \texttt{decisionRule};
  \item per il prossimo costrutto necessario, partire dalle guide precedenti, ricostruire prima il metodo generale e poi verificarlo con evidence corrente; prompt LLM e wrapper restano separati dalla semantica BA;
...
La Rebuild R7 mantiene completa la ricostruzione di \texttt{transfer}, \texttt{produce}, \texttt{realize}, \texttt{constrain} e \texttt{condition}. La Sezione 12 conserva come candidate la rilocazione del mapping in \texttt{operatorStructure.decisionRule}. DermaTriage supporta ora \texttt{eq}/\texttt{ne}, la candidate minima \texttt{gt} e la preservazione dei residuali; la rilocazione nel suo complesso resta non ammessa fino a regression Facial Access e checkpoint esplicito.
\end{ddtaopen}
\normalsize
%</PAGE-CONTENT:83>
