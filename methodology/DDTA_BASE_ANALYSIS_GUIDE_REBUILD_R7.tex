...
%<PAGE-HASH-DEFS>
\expandafter\def\csname PageMDfive1\endcsname{e36c5f64cea8931b2a6e5abcb558c338}
...
\expandafter\def\csname PageMDfive69\endcsname{47e9a7ceb31efd253c34453155337e71}
\expandafter\def\csname PageMDfive70\endcsname{c490bc72c060d4482b6f5599cc8eb8e1}
\expandafter\def\csname PageMDfive71\endcsname{eadac7583a078dbaec5090052eb85974}
\expandafter\def\csname PageMDfive72\endcsname{d6ee47ee9de0501f164fdb328fc69ddb}
\expandafter\def\csname PageMDfive73\endcsname{2205a8b6a451a27635473eac1f53b945}
\expandafter\def\csname PageMDfive74\endcsname{04d2d044867055a500a4f10de2caf3e4}
\expandafter\def\csname PageMDfive75\endcsname{f248e4c48942111fde0c866c423d5e24}
\expandafter\def\csname PageMDfive76\endcsname{4f78ba264c6eb0c06268caa9286bcf93}
...
\expandafter\def\csname PageMDfive83\endcsname{c4622709ab9c2bf97c8f3bce55724e42}
\expandafter\def\csname PageMDfive84\endcsname{c3ce06bf4610c41060c775c30b2fb397}
...
\textbf{Fase corrente:} R6 sospende nuove promotion di costrutti durante il source-preservation audit completo e il successivo audit di coerenza/connettivita' della BA accettata. La Rebuild R7 resta la superficie cumulativa corrente: \texttt{transfer}, \texttt{produce}, \texttt{realize}, \texttt{constrain}, lo scoped modifier \texttt{condition} e le candidate gia' documentate restano preservati; \texttt{selection}, sufficienza del modello dei contratti e rilocazione di \texttt{decisionRule} vengono riesaminati solo dopo i due audit globali.
}
\end{ddtaopen}
...
\textbf{Repository baseline} & \texttt{1502e6afc0a88a69fd976bb3b38de7e5fb8841a3} \\
...
\begin{ddtacore}[title={Comparison key sintetici nella candidate R7}]
La candidate normalizza le due relazioni gia' presenti nell'authority storica: \texttt{equals -> eq} e \texttt{notEquals -> ne}. Questa rinomina non estende la semantica. Il pressure test DermaTriage richiede inoltre la relazione stretta \texttt{confidence > 0.85}; il minimo incremento candidate e' quindi \texttt{gt}. I simboli \texttt{=}, \texttt{!=} e \texttt{>} sono rendering leggibili, non semantic key alternativi. Non vengono introdotti per simmetria \texttt{lt}, \texttt{le}, \texttt{ge}, range o aritmetica senza nuovi casi governati.
\end{ddtacore}
%</PAGE-CONTENT:69>
...
\begin{itemize}
  ...
  \item una residual condition non viene riscritta automaticamente come complemento booleano o negazione del branch precedente: ``other applicable HIGH cases'' non autorizza \texttt{NOT(gt)} ne' un comparison key \texttt{le};
  \item missing, invalid o out-of-domain restano non specificati quando la source non stabilisce che appartengano al residuale;
  \item \texttt{gt} e' strettamente ``maggiore di'': il valore esattamente uguale alla soglia non soddisfa il comparison; cio' non definisce da solo il branch residuale;
...
\subsection{Forbidden inference}
Non inferire completezza del mapping, default, precedence, short-circuit, gestione degli input mancanti, coercion dei tipi, range non documentati, error/failure handling, complementi booleani o comparison key ulteriori. Non trasformare valori locali come \texttt{P1} in BAReferent se non superano l'identity test BA1.
%</PAGE-CONTENT:70>
...
\begin{enumerate}
  ...
  \item La source governa il predicato complementare del residuale oppure soltanto il residuale stesso? Se governa soltanto il residuale, non normalizzarlo in \texttt{NOT(...)} o in un comparison inverso.
  \item I branch sono semanticamente disgiunti oppure servirebbe precedence/evaluation order non documentato?
  \item Le comparison richieste sono rappresentabili con i key ammessi dalla candidate corrente (\texttt{eq}, \texttt{ne}, \texttt{gt})? Se no, registrare pressure invece di inventare operatori di confronto.
  \item Il comportamento sulla soglia esatta, su input missing/invalid o fuori dominio e' governato? Se non lo e', lasciarlo \texttt{NOT SPECIFIED} e non usarlo per chiudere un branch residuale.
...
\begin{ddtacheck}[title={Procedura minima}]
Estrarre prima \texttt{produce} senza rule; stabilizzare actor/input/result; separare eventuale \texttt{condition} di applicabilita'; isolare poi soltanto i branch che selezionano il result; preservare valori, comparison e residual wording governati; testare soglia esatta e casi missing/invalid senza completarli per intuizione; infine confrontare la collocazione candidate annidata con l'attuale \texttt{decisionRule} top-level per assicurare che nessuna identita' o provenance necessaria venga persa.
\end{ddtacheck}
%</PAGE-CONTENT:71>
...
produce
  ...
  operatorStructure
    decisionRule
      branch
        when -> RequestClassification.classKind eq EXPRESS
        resultAssignment -> QueueClass = FAST
      branch
        when -> RequestClassification.classKind eq STANDARD
...
\texttt{FR-MR01-04-01} governa quattro selezioni e ha gia' superato il pressure test sui boundary case usati in review: soglia esatta, confidence missing/invalid, valori MEDIUM/LOW con confidence alta e scope image-based. La seguente resta una \emph{candidate authoring view}, non BA gia' accettata:

\begin{lstlisting}
produce
  actor  -> OperationalPriorityDerivation
  input  -> CaseUrgency
  ...
  operatorStructure
    decisionRule
      branch
        when -> CaseUrgency eq HIGH AND TriageConfidence gt 0.85
        resultAssignment -> OperationalTriagePriority = P1
      branch
        when -> other applicable cases within CaseUrgency eq HIGH
        resultAssignment -> OperationalTriagePriority = P2
      branch
        when -> CaseUrgency eq MEDIUM
        resultAssignment -> OperationalTriagePriority = P3
      branch
        when -> CaseUrgency eq LOW
        resultAssignment -> OperationalTriagePriority = P4
\end{lstlisting}

Il secondo branch preserva un residuale governato: non equivale a \texttt{TriageConfidence le 0.85}, non copre automaticamente missing/invalid e non e' un \texttt{ELSE} globale. \texttt{gt} e' l'unico comparison key nuovo richiesto da questo caso.
%</PAGE-CONTENT:72>
...
8. Preserve explicit residual wording such as "other HIGH cases" in its exact scope.
   Do not rewrite it as NOT(previous condition) or as an inferred inverse comparison.
9. In this R7 candidate use eq/ne as the normalized spellings of the current
   equals/notEquals semantics. gt is the only new candidate comparison currently
   justified by DermaTriage. Any other comparison returns VOCABULARY_PRESSURE.
10. Do not infer handling for threshold equality, missing, invalid or out-of-domain
    inputs unless the source explicitly governs it.
11. Do not turn allowed-domain semantics into result selection; review constrain.
12. If the rule needs independent identity/reuse/provenance/change lifecycle,
    return IDENTITY_PRESSURE and stop local nesting.
13. Do not invent BAReferents, properties, typed values, boolean DSL nodes or metadata.

Return EXACTLY the review wrapper and candidate BA shape shown next.
\end{lstlisting}

\begin{ddtaprinciple}[title={Human review remains mandatory}]
La review umana deve verificare tre confini distinti: \texttt{condition} contro result selection, residuale contro complemento inferito e local structure contro independent proposition identity. La presenza linguistica di ``if'', ``otherwise'' o di un simbolo di confronto non decide da sola nessuno di questi confini.
\end{ddtaprinciple}
%</PAGE-CONTENT:73>
...
RESULT_SELECTION_ASSESSMENT
  ...
  comparisonStatus = CURRENT_EQ_NE | CANDIDATE_GT | VOCABULARY_PRESSURE
  residualStatus = NONE | SOURCE_GROUNDED_RESIDUAL | AMBIGUOUS_SCOPE
...
I campi di assessment appartengono al wrapper di review, non alla BA. \texttt{CURRENT\_EQ\_NE} indica la normalizzazione nominale delle semantiche storiche \texttt{equals/notEquals}; \texttt{CANDIDATE\_GT} segnala l'uso della sola estensione stretta richiesta dal pressure test corrente. \texttt{SOURCE\_GROUNDED\_RESIDUAL} non autorizza a materializzare un complemento booleano.
...
Blu = produzione; ambra = applicability; grigio tratteggiato = result selection interna. Il collegamento grigio non e' dataflow. Il simbolo \texttt{>} e' il rendering leggibile del comparison key candidate \texttt{gt}; il branch ``HIGH, other applicable cases`` visualizza un residuale source-grounded e non un complemento \texttt{le 0.85}. La figura non promuove la rilocazione: visualizza il \texttt{decisionRule} annidato candidate da sottoporre a regression prima di modificare l'authority BA.
\end{ddtacheck}
%</PAGE-CONTENT:76>
...
%<PAGE-DESC:83>Appendice temporanea - roadmap e registro dei costrutti da integrare/studiare
%<PAGE-CONTENT:83>
\PageHashFooter{\PageMDfive{83}}
\TemporaryRebuildBanner
\subsection{Roadmap e registro temporaneo dei costrutti}
\fontsize{8.15}{9.05}\selectfont
Questo registro e' la coda esplicita della nuova guida cumulativa. Non promuove alcun costrutto: conserva cio' che e' gia' ricostruito, cio' che deve essere integrato e cio' che resta da studiare, usando il Source Register R25 e i ledger di review come evidence non normativa.

\vspace{0.6mm}
\begin{ddtaopen}[title={Registro temporaneo - candidate/strutture da integrare nella guida}]
\fontsize{6.7}{7.35}\selectfont
\renewcommand{\arraystretch}{1.03}
\setlength{\tabcolsep}{2.4pt}
\begin{tabularx}{\textwidth}{L{28mm}L{43mm}L{47mm}Y}
\toprule
\textbf{Costrutto} & \textbf{Classe} & \textbf{Stato corrente} & \textbf{Azione nel rebuild} \\
\midrule
\texttt{decisionRule} & local \texttt{operatorStructure} di \texttt{produce} (rilocazione candidate) & \texttt{CANDIDATE / NOT ADMITTED} & Sez. 12 gia' presente; completare regression Facial Access e checkpoint prima di qualsiasi promotion. \\
\texttt{selection} & reusable local / second-level structure & \texttt{REUSABLE\_LOCAL\_STRUCTURE / NON-NORMATIVE}; current BA non modificata & \textbf{TO INTEGRATE}: prossimo costrutto di secondo livello da ricostruire nella guida cumulativa. Evidence: PR-13 con controlli FR-06/FR-18/FR-20. Firma generale \texttt{NOT FROZEN}. \\
\texttt{CC-02 provideService} & candidate construct & \texttt{TESTED\_POSITIVE / NOT\_ADMITTED} & Hold: formal gate G1--G8 / evidence review prima di eventuale integrazione. \\
\texttt{CC-03 storedIn} & candidate construct & \texttt{TESTED\_POSITIVE / NOT\_ADMITTED} & Hold: riesame formale dell'associazione at-rest; non confondere con transfer o write/persist. \\
\texttt{CC-04 initiate} & candidate construct & \texttt{TESTED\_POSITIVE / NOT\_ADMITTED} & Hold: formal gate G1--G8 e regression cross-corpus prima di eventuale integrazione. \\
\bottomrule
\end{tabularx}
\end{ddtaopen}

\vspace{0.6mm}
\begin{ddtaopen}[title={Pressures / delta ancora da studiare}]
\fontsize{6.7}{7.35}\selectfont
\texttt{PR-01} execution identity; \texttt{PR-02} pipeline composition/order; \texttt{PR-04} boundary/interaction; \texttt{PR-05/06} ordered/scalar comparison; \texttt{PR-07} structured information contract; \texttt{PR-09} acquisition/refresh; \texttt{PR-12} negative implication/non-sufficiency; \texttt{PR-14} applicability/configuration binding; \texttt{CMD-OP04} observe.result removal; \texttt{CMD-OP05} transition state/value refinement; \texttt{OBS-OT-01} operation-target/effect-scope. Nessuna voce puo' scomparire senza disposition esplicita.
\end{ddtaopen}

\begin{ddtaopen}[title={Stop point di questa R7}]
\fontsize{6.8}{7.45}\selectfont
La Rebuild R7 mantiene completa la ricostruzione di \texttt{transfer}, \texttt{produce}, \texttt{realize}, \texttt{constrain} e \texttt{condition}. \texttt{decisionRule} resta la prima struttura candidate gia' descritta. \texttt{selection} e' ora registrata esplicitamente come reusable local / second-level structure R25 da integrare nella prossima revisione cumulativa, senza trasformarla in top-level operator e senza congelarne prematuramente la firma. Gli altri candidate/open item restano non ammessi fino alla rispettiva review.
\end{ddtaopen}
\normalsize
%</PAGE-CONTENT:83>
...
\IntegrityRow{83}{Appendice temporanea - roadmap e registro dei costrutti da integrare/studiare}{\PageMDfive{83}}
